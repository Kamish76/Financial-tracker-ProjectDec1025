...
/**
 * Client-side utility to calculate filtered transaction statistics
...
export function calculateFilteredStats(transactions: any[]): FilteredStats {
  ...
  for (const tx of transactions) {
    ...
    if (type === 'income') {
      ...
      stats.byType.expense_business += amount
    } else if (type === 'expense_personal') {
      stats.byType.expense_personal += amount
    } else if (type === 'held_allocate') {
      stats.byType.held_allocate += amount
    } else if (type === 'held_return') {
      stats.byType.held_return += amount
    }
  }

  // Gross Profit = income - business expenses (before personal expenses and salary deductions)
  stats.grossProfit = stats.byType.income - stats.byType.expense_business
