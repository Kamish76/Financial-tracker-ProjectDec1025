...
export async function fetchTransactionsWithFilters(
  ...
  // Enrich with user/category data
  const enrichedTransactions = await Promise.all(
    transactions.map(async (tx) => {
      const [fundedByUser, recorder, categoryRef] = await Promise.all([
        tx.funded_by_user_id
          ? admin
              .from("profiles")
              .select("id, full_name")
              .eq("id", tx.funded_by_user_id)
              .maybeSingle()
      ...
      return {
        ...tx,
        funded_by_user: fundedByUser.data,
        recorder: recorder.data,
        category_ref: categoryRef.data,
      }
    })
  )

  return { transactions: enrichedTransactions, nextCursor, hasMore }
}
